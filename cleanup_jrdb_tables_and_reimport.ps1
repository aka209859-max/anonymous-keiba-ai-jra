...
Write-Host "`n再登録後の確認コマンド:" -ForegroundColor Yellow
Write-Host "以下のコマンドをコピー＆ペーストして実行してください：" -ForegroundColor White
Write-Host ""
...
Write-Host "& `"C:\Program Files\PostgreSQL\16\bin\psql.exe`" -U postgres -d pckeiba -c `"SELECT race_shikonen, LENGTH(race_shikonen) AS len, keibajo_code, race_bango, bamei FROM jrd_kyi ORDER BY race_shikonen DESC LIMIT 10;`"" -ForegroundColor Cyan
